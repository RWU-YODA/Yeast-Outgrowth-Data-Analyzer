...
plotAllCurves(survivalMatrix, timePointTimes, strainNames)
...
# Plot all survival curves on the same graph. 
plotAllCurves <- function(survivalMatrix, timePointTimes, nameVector)
{
  plot(
    timePointTimes,
    survivalMatrix[,1],
    main = "Combined Survival Curve",
    xlab = "Time Point (Day) ",
  ...
  lines(timePointTimes, survivalMatrix[,1], type = "o")
  
  
  for(i in 2: length(nameVector))
  {
    points(timePointTimes,
         survivalMatrix[,i],
         pch = i,
         col = i)
    
    lines(timePointTimes, survivalMatrix[,i], type = "o", col=i)
    
  }
  
  legend(10,80, nameVector,# places a legend at the appropriate place c("Health","Defense"), # puts text in the legend
         
         lty=c(1,1), # gives the legend appropriate symbols (lines)
         
         lwd=c(2.5,2.5),col=1:length(nameVector))
